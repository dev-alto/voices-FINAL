...
function pointAddressed(point: string, history: string[]): boolean {
  ...
  return (
    overlap >= Math.min(2, Math.max(1, Math.floor(pointTokens.size * 0.2)))
  );
}

function chooseNextPoint(
  points: Array<{ text: string; importance: number }>,
  history: string[]
) {
...
export const generateAgentResponse = (
  ...
  if (!response) {
    ...
    const extrasText = `${context?.userExtras || ""} ${
      context?.scenarioBasePrompt || ""
    }`.trim();
    ...
    if (allTokens.length > 0) {
      ...
      const fallback = fu.length
        ? fu[Math.floor(Math.random() * fu.length)]
        : "Could you elaborate?";
  ...
  if (!response) {
    ...
    const pool =
      conversationHistory.length > 2 ? [...templates, ...followUps] : templates;
    const candidate =
...
export const canGenerateAgentResponse = (
  state: ConversationTurnState
): boolean => {
  // Circuit breaker: prevent runaway agent responses
  if (state.consecutiveAgentMessages >= 2) {
    console.warn(
      `[TurnTaking] Circuit breaker triggered: ${state.consecutiveAgentMessages} consecutive agent messages`
    );
    return false;
  }
  ...
  const canGenerate =
    state.lastSpeaker === "user" && state.awaitingUser === false;

  console.log(
    `[TurnTaking] Session ${state.sessionId} - Can generate: ${canGenerate}, lastSpeaker: ${state.lastSpeaker}, awaitingUser: ${state.awaitingUser}, turnIndex: ${state.turnIndex}`
  );

  return canGenerate;
};

export const updateStateOnUserMessage = (
  state: ConversationTurnState,
  messageId?: string
  ...
  console.log(
    `[TurnTaking] User spoke - Session ${updated.sessionId}, Turn ${updated.turnIndex}, MessageId: ${messageId}`
  );
...
export const advanceAfterAgentResponse = (
  ...
  console.log(
    `[TurnTaking] Agent ${agentId} spoke - Session ${updated.sessionId}, Turn ${updated.turnIndex}, Consecutive agent messages: ${updated.consecutiveAgentMessages}`
  );
...
class DeduplicationCache {
  ...
  isDuplicate(
    ...
    if (sessionCache.has(hash)) {
      console.log(
        `[Deduplication] Duplicate detected for session ${sessionId}, hash: ${hash}`
      );
      return true;
    }
...
function uniqueAgentsFromMessages(
  messages: Message[]
...
export function selectAgentToSpeak(
  ...
  if (chosenStrategy === "active-host") {
    const host = availableAgents.find(
      (a) => a.name.toLowerCase() === DEFAULT_HOST_NAME.toLowerCase()
    );
    if (host) {
      selected = host;
      console.log(
        `[SpeakerSelection] Selected host agent: ${host.name} (${host.id})`
      );
    }
  }
  ...
  if (!selected) {
    ...
    const rrIdx =
      availableAgents.length > 0
        ? countAgentMessages(messages) % availableAgents.length
        : 0;
    selected = availableAgents[rrIdx] || null;
    if (selected) {
      console.log(
        `[SpeakerSelection] Selected agent via round-robin: ${selected.name} (${selected.id})`
      );
    }
  }
