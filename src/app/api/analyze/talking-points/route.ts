...
export async function POST(request: NextRequest) {
  try {
    ...
    const body: GeneratePointsRequest = await request.json();
    const {
      context,
    ...
    if (lastMessageRole && lastMessageRole !== "user") {
      console.warn(
        `[Talking Points API] Rejected generation request with lastMessageRole: ${lastMessageRole}`
      );
      return NextResponse.json(
        {
          error: "Talking points generation is only triggered by user messages",
          skipped: true,
        },
...
- ${
      presentational
        ? "This is a presentational scenario. Prioritize an arc: objective, key sections, transitions, and conclusion."
        : "This is an interactive scenario. Prioritize goal-oriented, conversational points and checkpoints."
    }
...
${context}`;
    ...
    if (!parsed) {
      return NextResponse.json(
        {
          error: "Failed to parse talking points from model output.",
    ...
    return NextResponse.json({
      points,
      meta: { model: "gemini-2.0-flash-exp", count: points.length },
    });
  } catch (error: any) {
...
function safeParsePoints(text: string | undefined | null): RawPoint[] | null {
  ...
  try {
    ...
    if (obj && Array.isArray((obj as any).points))
      return (obj as any).points as RawPoint[];
  } catch {}
  ...
  if (lines.length) {
    ...
    for (const line of lines) {
      ...
      if (m) {
        ...
        if (textVal)
          points.push({ text: textVal, importance: clamp(imp, 1, 5) });
      }
    }
