...
export async function POST(request: NextRequest) {
  try {
    ...
    if (lastMessageRole && lastMessageRole !== "user") {
      console.warn(
        `[Flow API] Rejected flow generation request with lastMessageRole: ${lastMessageRole}`
      );
      return NextResponse.json(
        {
          error: "Flow generation is only triggered by user messages",
          skipped: true,
        },
...
- ${
      presentational
        ? "This is a presentational scenario. Emphasize clarity, scaffolding, and logical structure."
        : "This is an interactive scenario. Keep the structure brief and flexible."
    }
...
CONTEXT:\n${context}`;
    ...
    if (!parsed) {
      return NextResponse.json(
        {
          error: "Failed to parse flow from model output.",
    ...
    if (normalized.sections.length > max)
      normalized.sections = normalized.sections.slice(0, max);
    if (normalized.sections.length < min) {
      const needed = min - normalized.sections.length;
      for (let i = 0; i < needed; i++) {
        normalized.sections.push(
          blankSection(`Section ${normalized.sections.length + 1}`)
        );
      }
    }
...
function normalizeFlow(raw: RawFlow) {
  const normSection = (s?: RawFlowSection, fallbackTitle?: string) => {
    const title =
      (s?.title || fallbackTitle || "").toString().trim().slice(0, 120) ||
      fallbackTitle ||
      "Section";
    ...
    return {
      ...
      goals: goals.length
        ? goals
        : ["State the objective", "Set audience expectations"],
    };
  };
...
function blankSection(title: string) {
  return {
    id: randomUUID(),
    title,
    goals: ["Add a brief goal", "Add another brief goal"],
  };
}
